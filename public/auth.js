...
// Provider list drives both the login screen's buttons and which providers are wired up. Adding
// Naver later (once it exists as a Supabase-native or custom OIDC provider) is flipping `enabled`
// to true - no UI rebuild needed (요청 2).
export const OAUTH_PROVIDERS = [
  { id: 'google', label: 'Google로 계속하기', enabled: true },
  { id: 'kakao', label: 'Kakao로 계속하기', enabled: true },
  { id: 'naver', label: 'Naver로 계속하기 (준비 중)', enabled: false }
];
...
export async function signOut(config, session, fetchImpl = fetch, storage = localStorage) {
  if (config && session) {
    try {
      await fetchImpl(new URL('/auth/v1/logout', config.supabaseUrl), { method: 'POST', headers: { apikey: config.anonKey, Authorization: `Bearer ${session.accessToken}` } });
    } catch { /* Local session is still cleared below - logout must never get "stuck" on a network error. */ }
  }
...
export function decideGateScreen(session, profile) {
